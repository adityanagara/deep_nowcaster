...
sites = ['txda','txdc','txde','txco','txty','txhm','txwa','txwf','txhm','txno','txpa']
...
for s in sites:
    plt.figure()
    plt.plot(UCARvals[UCARvals.site.values == s].IPW.values,'r-',label = 'UCAR published')
    plt.plot(vals[vals[:,0] == s,1:].reshape(48,).astype('float'),'b-', label = 'Locally processed')
    plt.title('Varification for site: ' + s)
