...
def build_DCNN_softmax(input_var = None,stride = 2):
...
def build_DCNN_maxpool_softmax(input_var = None):
        ...
        l_maxpool = dnn.MaxPool2DDNNLayer(incoming = l_conv1,stride = (2,2), pad = 'full')
...
def conv_net(tr_block,val_block,num_epochs,exp_no,load_model_weights = False,model_file = '../output/2CNN_2maxpool_neural_network_refl_0_90.pkl'):
    #------------------------------------------
    # Model
    ...
    val_fn = theano.function([input_var, target_var], [test_accuracy,test_loss])
    #------------------------------------------
    performance_metrics = {}
    base_path = '../data/TrainTest/points/'
...
#            X_train_full_data.append((X_train,Y_train))
        ...
        for X_,Y_ in iterate_minibatches(X_train,Y_train, 250, shuffle = True):
            train_err += train_fn(X_.astype('float32'),Y_.reshape(-1,))
            train_batches += 1
...
#            x_val_batch = x_val_batch.astype('float')
            temp = val_fn(x_val_batch.astype('float32'),y_val_batch.reshape(-1,))
            val_acc += temp[0]
            val_loss += temp[1]
            val_batches_ctr+=1
        ...
        if (ep+ 1) % 10 == 0:
            ...
            network_file.close()
            f1 = file('../output/performance_metrics_' + network_file_name + str(exp_no) + '.pkl','wb')
