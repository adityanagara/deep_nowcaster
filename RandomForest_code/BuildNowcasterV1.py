...
#
#for px in range(OutputMatrix.shape[0]):
#    # 960 time steps in training case 20*48 = 960
#    TrainMatrix = np.concatenate((FeatureMatrix[:960,:],OutputMatrix[px,:960].reshape(-1,1)),axis=1)
#
#    TestMatrix = np.concatenate((FeatureMatrix[960:,:],OutputMatrix[px,960:].reshape(-1,1)),axis=1)
#
#
#    x = TrainMatrix[:,:-1]
#
...
#        ctr+=1
#        roc_auc = auc(fpr,tpr)
#        print '%d) Area Under the curve for pixel point %d,%d = %f'%(ctr,PixelPoints[px,0],PixelPoints[px,1],roc_auc)
#    
#
#    plt.figure()
#    plt.plot(fpr,tpr)
...
# 2. Feature vector that only includes IPW fields
