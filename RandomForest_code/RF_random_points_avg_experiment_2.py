...
num_trees = [10,20,50,100,200,250,300,500]

avg_f1_list = []
for n_est in num_trees:
    f1_list = []
    for train_index,test_index in kf:
        X_train, X_test = data_set[train_index,4:8],data_set[test_index,4:8]
        Y_train, Y_test = data_set[train_index,-1],data_set[test_index,-1]

        mdl = RandomForestClassifier(n_estimators = n_est,n_jobs = -1,class_weight = 'auto')
...
Num_trees_CV_file = file('model_metrics/RF_Num_Trees_CV_Score_refl.pkl')
cPickle.dump(avg_f1_list,Num_trees_CV_file)
Num_trees_CV_file.close()
print 'Done!'
