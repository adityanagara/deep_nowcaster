...
y_hats = np.around(y_hats,3)

doy_128_129_train_x = np.zeros((900,92,4))
doy_132_test_x = np.zeros((900,44,4))

for point in range(y_hats.shape[0]):
    
    temporal_list = []
    
    temp_vals = y_hats[point,:96,1]
    ...
    for idx in range(temp_vals.size):
        if idx+3 < 95:
            ...
            doy_128_129_train_x[point,idx,:3] = temp_vals[idx:idx+3]
            doy_128_129_train_x[point,idx,3] = temp_out[idx+4]
    
    test_temp_vals = y_hats[point,96:,1]
    ...
    for idx2 in range(test_temp_vals.size):
        if idx2 + 3 < 47:
            ...
            doy_132_test_x[point,idx2,:3] = test_temp_vals[idx2:idx2+3]
            doy_132_test_x[point,idx2,3] = test_temp_out[idx2+4]
